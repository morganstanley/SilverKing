...
public class Util {
  ...
  //  if we use this method with the commented out portion, we get these outputs instead of the actual contents of
  //  the array: [[I@579bb367]
  //    it has to do with primitives and generics:
  //       if we are using the method inline, we are calling it directly with the primitive array, like int[] a, so
  //       it works just fine
  //       if we are using this method, we are using a generic T, and passing createString(a), so it's getting a
  //       little confused
  @SafeVarargs
