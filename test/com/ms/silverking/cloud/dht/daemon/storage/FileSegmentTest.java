...
public class FileSegmentTest {
    ...
    private StorageParameters getStorageParameters(byte[] value) {
        ...
        return new StorageParameters(version, uncompressedSize, compressedSize, lockSeconds, ccss, checksum, valueCreator, creationTime);
    }
    ...
    public void test() {
        ...
        numKeys = 2;
        valuesPerKey = 1;
        _test(segmentNumber, "0", EnumSet.of(SegmentTest.Write), numKeys, dataSegmentSize, valuesPerKey, new FileSegmentStorageFormat(0));
        _test(segmentNumber, "0", EnumSet.of(SegmentTest.Write), numKeys, dataSegmentSize, valuesPerKey, new FileSegmentStorageFormat(2));
    }
    ...
    public void test1() {
        ...
        numKeys = 2;
        valuesPerKey = 1;
        _test(segmentNumber, "1", EnumSet.of(SegmentTest.Write, SegmentTest.Read), numKeys, dataSegmentSize, valuesPerKey, new FileSegmentStorageFormat(0));
        _test(segmentNumber, "1", EnumSet.of(SegmentTest.Write, SegmentTest.Read), numKeys, dataSegmentSize, valuesPerKey, new FileSegmentStorageFormat(2));
    }
    ...
    public void test2() {
        ...
        numKeys = 2;
        valuesPerKey = 4;
        _test(segmentNumber, "2", EnumSet.of(SegmentTest.Write, SegmentTest.Read), numKeys, dataSegmentSize, valuesPerKey, new FileSegmentStorageFormat(0));
        _test(segmentNumber, "2", EnumSet.of(SegmentTest.Write, SegmentTest.Read), numKeys, dataSegmentSize, valuesPerKey, new FileSegmentStorageFormat(2));
    }
    
    private void _test(int segmentNumber, String id, Set<SegmentTest> tests, int numKeys, int dataSegmentSize, int valuesPerKey, FileSegmentStorageFormat storageFormat) {
        try {
            FileSegment fs;
            DHTKey[]    keys;
            String[]    values;
            ...
            if (tests.contains(SegmentTest.Read)) {            
                fs = FileSegment.openForDataUpdate(nsDir, segmentNumber, dataSegmentSize, FileSegment.SyncMode.NoSync, _nsOptions, SegmentIndexLocation.RAM, FileSegment.SegmentPrereadMode.Preread);
                for (DHTKey key : keys) {
