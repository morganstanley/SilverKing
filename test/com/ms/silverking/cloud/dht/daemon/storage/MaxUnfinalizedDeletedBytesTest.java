...
public class MaxUnfinalizedDeletedBytesTest {
  ...
  private NamespaceStore createFakeNS(int testSegmentSizeBytes) {
    NamespaceOptions testMutableLruNsOpts = DHTConstants.defaultNamespaceOptions.consistencyProtocol(
        ConsistencyProtocol.LOOSE).versionMode(NamespaceVersionMode.SYSTEM_TIME_NANOS).namespaceServerSideCode(
        NamespaceServerSideCode.singleTrigger(LRUTrigger.class)).valueRetentionPolicy(
        new LRURetentionPolicy(1000, 1)).segmentSize(testSegmentSizeBytes);
    NamespaceProperties nsprop = new NamespaceProperties(testMutableLruNsOpts);

    return new NamespaceStore(1, fakeNsDir, NamespaceStore.DirCreationMode.DoNotCreateNSDir, nsprop, null, mockMg,
        mockRingMaster, true, new ConcurrentHashMap<>(), new LRUReapPolicy(true, true, true, 500, 1000, 1000, true),
        fakeFinalization, mockFileSegmentCompactor);
  }
  ...
  public void testMaxUnfinalizedDeletedBytes() {
    ...
    if (tmp < NamespaceOptions.maxMaxValueSize) {
      ...
      if (tmp2 < Integer.MAX_VALUE) {
        ...
        // 3. try to adjust loopTime (will be 4 loops, if both proposedSegmentSize and
        // proposedSegmentNumDeletedEachTime are maximum)
        proposedLoopTime =
            (int) (maxUnfinalizedDeletedBytes / ((long) proposedSegmentNumDeletedEachTime * (long) proposedSegmentSize));
      }
    }
