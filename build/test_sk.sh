...
typeset output_filename=$(f_getTestSk_RunOutputFilename)
{
    f_startLocalTimer;
    date;
    f_runTestAntScript "testSkOnly-small" "$output_filename" "$TEST_SILVERKING_SMALL_EXPECTED_MIN_COUNT" "$(f_getSkSmallExpectedCount)";
    typeset vars="
    ...
    "
    f_runTestAntScript "testSkOnly-large" "$output_filename" "$TEST_SILVERKING_LARGE_EXPECTED_MIN_COUNT" "$(f_getSkLargeExpectedCount)" "$vars";
