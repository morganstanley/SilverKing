...
int16_t SKPutOptions::getLockSeconds() const {
    return (int16_t)((PutOptions*)pImpl)->getLockSeconds() ; 
}
...
int64_t SKPutOptions::previousVersionNonexistent() {
    return PutOptions::previousVersionNonexistent();
}

int64_t SKPutOptions::previousVersionNonexistentOrInvalid() {
    return PutOptions::previousVersionNonexistentOrInvalid();
}
        ...
        SKVal * userData)
{
    ...
    pImpl = new PutOptions(java_new<PutOptions>(controller, targets, *pCompr, *pCt, checksumCompressedValues,
                version, requiredPreviousVersion, lockSeconds, fragmentationThreshold, byteArray)); 
    delete pCompr;
    delete pCt;
}
