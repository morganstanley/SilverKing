...
CacheStoreResult fbc_store_dht_value(FileBlockCache *fbCache, FileBlockID *fbid, SKVal *pRVal,
                                    uint64_t modificationTimeMillis = CACHE_NO_MODIFICATION_TIME);
CacheStoreResult fbc_store_raw_data(FileBlockCache *fbCache, FileBlockID *fbid, void *data, size_t size, int replace = FALSE, uint64_t modificationTimeMillis = CACHE_NO_MODIFICATION_TIME);
...
void fbc_store_error(FileBlockCache *fbCache, FileBlockID *key, int errorCode, uint64_t modificationTimeMillis, uint64_t timeoutMillis);
