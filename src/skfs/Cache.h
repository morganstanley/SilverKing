...
CacheReadResult cache_read(Cache *cache, void *key, size_t keySize, unsigned char *buf, 
                           ...
                           uint64_t minModificationTimeMillis = 0,
...
CacheStoreResult cache_store_dht_value(Cache *cache, void *key, int keySize, SKVal *pRVal, 
    uint64_t modificationTimeMillis = CACHE_NO_MODIFICATION_TIME, uint64_t timeoutMillis = CACHE_NO_TIMEOUT);
CacheStoreResult cache_store_raw_data(Cache *cache, void *key, int keySize, void *data, size_t length, int replace = FALSE, uint64_t modificationTimeMillis = CACHE_NO_MODIFICATION_TIME, uint64_t timeoutMillis = CACHE_NO_TIMEOUT);
void cache_store_active_op(Cache *cache, void *key, int keySize, ActiveOp *op);
void cache_store_error(Cache *cache, void *key, int keySize, int errorCode, int notifyActiveOps_noStorage /*= FALSE*/, uint64_t modificationTimeMillis, uint64_t timeoutMillis);
