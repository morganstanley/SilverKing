...
public class Constraint {
    ...
    public static void checkBounds(int min, int max, int i, String msg) {
        if (max < min) {
            String possibleFailure = max < 0 ? "the integer value for max bound is possibly overflow" : "the min/max bound might be specified in wrong order (or with wrong value)";
            throw new ConstraintViolationException("max bound [" + max + "] is smaller than min bound [" + min + "], " + possibleFailure);
        }
