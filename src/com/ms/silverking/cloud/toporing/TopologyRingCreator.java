...
public class TopologyRingCreator {
  ...
  private List<NodeAllocationState> getInitialAllocationStates(List<NodeAllocationState> nodeAllocationStates_final,
    ...
    for (int i = 0; i < nodeAllocationStates_final.size(); i++) {
      ...
      if (debug) {
        System.out.printf("node %s\toldAllocation %d\tallocationLimit %d\n",
  ...
  private void allocateSubPolicyMember2(SingleRing sourceRing, ProtoRegionList prList, SubPolicyMember member,
    ...
    if (debug) {
      System.out.println("TopologyRingCreator.allocateSubPolicyMember2()");
    }
        ...
        while (!successful && attemptIndex++ < maxShuffleAttempts) {
          ...
          try {
            for (int _nodeIndex = 0; _nodeIndex < nodes.size(); _nodeIndex++) {
              ...
              ringspaceAllocated = 0;
              sanityCheckAllocations(prList, nodes, nodeRegionSizes, replicaIndex, nodeIndex,
  ...
  public static TopologyRing removeExcludedMembers(TopologyRing oldRing, ExclusionSet exclusionSet)
      throws InvalidRingException {
    return removeExcludedMembers(oldRing, dummySetNodes(exclusionSet.getServers()));
  }
