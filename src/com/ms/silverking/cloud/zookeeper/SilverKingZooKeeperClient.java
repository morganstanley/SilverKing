...
public class SilverKingZooKeeperClient {
  ...
  private static CuratorWithBasePath createAndSetCurator(ZooKeeperConfig zkConfig, int sessionTimeoutMs) {
    synchronized (curatorPoolLock) {
      CuratorFramework curator;
      CuratorWithBasePath curatorWithSubPath;

      curator = CuratorFrameworkFactory.builder().ensembleProvider(
          new ImmutableEnsembleProvider(zkConfig.getConnectString())).retryPolicy(
          defaultRetryPolicy).dontUseContainerParents().sessionTimeoutMs(sessionTimeoutMs).build();
  ...
  public static void setCurator(ZooKeeperConfig zkConfig, CuratorWithBasePath curatorWithBasePath)
    ...
    synchronized (curatorPoolLock) {
      ...
      base = currConnStr.substring(currConnStr.indexOf('/'));
      if (!chroot.startsWith(base)) {
        throw new IllegalArgumentException(
            "Given curator is using [" + currConnStr + "] which cannot work with chroot [" + chroot + "]");
      }
      curatorPool.put(zkConfig, curatorWithBasePath);
      Log.warningf("[" + zkConfig + "] is bound with curator [" + curatorWithBasePath + "]");
    }
  }
  ...
  public static class ImmutableEnsembleProvider extends FixedEnsembleProvider {
    ...
    public void setConnectionString(String connectionString) {
      Log.warning("Ignoring setConnectionString {}", connectionString);
    }
  }
  ...
  public SilverKingZooKeeperClient(ZooKeeperConfig zkConfig, int sessionTimeoutMs) {
    ...
    if (sharedInstance == null) {
      this.curatorWithBasePath = createAndSetCurator(zkConfig, sessionTimeoutMs);
    } else {
  ...
  public void setEphemeral(String path, byte[] data) throws KeeperException {
    try {
      curatorWithBasePath.getCurator().create().orSetData().withMode(CreateMode.EPHEMERAL).forPath(
          curatorWithBasePath.getResolvedPath(path), data);
    } catch (Exception ex) {
  ...
  public Stat set(String path, byte[] data, int version) throws KeeperException {
    try {
      return curatorWithBasePath.getCurator().setData().withVersion(version).forPath(
          curatorWithBasePath.getResolvedPath(path), data);
  ...
  public String create(String path, byte[] data, CreateMode createMode) throws KeeperException {
    try {
      return curatorWithBasePath.getCurator().create().withMode(createMode).forPath(
          curatorWithBasePath.getResolvedPath(path), data);
    } catch (InterruptedException ie) {
      throw new RuntimeException("panic");
    } catch (Exception ex) {
  ...
  public CompletableFuture<Void> deleteAsync(String path) throws KeeperException {
    ...
    try {
      ...
      curatorWithBasePath.getCurator().delete().withVersion(ANY_VERSION).inBackground(
          (CuratorFramework client, CuratorEvent event) -> op.complete(null)).forPath(
  ...
  public int getInt(String path, Watcher watcher) throws KeeperException {
    ...
    try {
      data = curatorWithBasePath.getCurator().getData().usingWatcher(watcher).forPath(
          curatorWithBasePath.getResolvedPath(path));
    } catch (InterruptedException ie) {
      throw new RuntimeException("panic");
    } catch (Exception ex) {
  ...
  private Map<String, byte[]> syncGetPathsData(Set<String> paths, Watcher watcher) throws KeeperException {
    ...
    for (String path : paths) {
      try {
        byte[] data = curatorWithBasePath.getCurator().getData().usingWatcher(watcher).forPath(
            curatorWithBasePath.getResolvedPath(path));
        if (data != null) {
          Log.warning("Sync zk get success: ", path);
          dataMap.put(path, data);
        } else {
          Log.logErrorWarning(new Exception("No data has been found in path [" + path + "]"));
        }
      } catch (Exception ex) {
  ...
  public Map<String, byte[]> getByteArrays(String basePath, Set<String> children, Watcher watcher,
  ...
  public List<String> getChildren(String path, Watcher watcher) throws KeeperException {
    try {
      return curatorWithBasePath.getCurator().getChildren().usingWatcher(watcher).forPath(
          curatorWithBasePath.getResolvedPath(path));
    } catch (InterruptedException ie) {
      throw new RuntimeException("panic");
    } catch (Exception ex) {
  ...
  public static SilverKingZooKeeperClient getZooKeeperWithRetries(ZooKeeperConfig zkConfig, int sessionTimeout,
      int connectAttempts) {
    ...
    do {
      try {
        ...
        Log.logErrorWarning(ie, "Interrupted while connecting to ZooKeeper");
      } catch (Exception e) {
        if (curAttempt >= connectAttempts) {
          Log.logErrorWarning(e, "Exception while connecting to ZooKeeper, all retries exhausted");
          throw e;
        } else {
          Log.logErrorWarning(e, "Caught exception while connecting to ZooKeeper, will retry after wait period");
        }
      }
      curAttempt++;
  ...
  public void createAllNodes(String path) throws KeeperException {
    try {
      curatorWithBasePath.getCurator().create().orSetData().creatingParentsIfNeeded().forPath(
          curatorWithBasePath.getResolvedPath(path), "".getBytes());
    } catch (Exception ex) {
  ...
  // Why is this reinventing KeeperException?
