...
public class PolicyParser {
    ...
    public StoragePolicyGroup parsePolicyGroup(String def, long version) throws PolicyParseException {
        ...
        return StoragePolicyGroup.create(policyGroupName, rootClassAndName, 
                                         storagePolicies, version);
    }
    ...
    private SubPolicy parseSubPolicy(LabeledBlock block) throws PolicyParseException {
        return new SubPolicy(parseReplicationType(block.getLabel()), 
                SubPolicyMember.parseMultiple(block.getBlock()));
    }
    ...
    public static void main(String[] args) {
        try {
            if (args.length == 0) {
                System.err.println("args: <policyFile>");
            } else {
                File  policyFile;
                ...
                System.out.println(storagePolicyGroup);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
