...
public class RSSEnforcer implements SafetyEnforcer {
    ...
    public RSSEnforcer(PropertiesHelper ph, Terminator terminator, ArgusOptions options, PeerWarningModule udpModule) {
        ...
        candidateMinThreashold = ph.getLong(propCandidateMinThreashold, defaultCandidateMinThreasholdMB, ParseExceptionAction.DefaultOnParseException) * ONEMB; 
        freeMemoryLimit        = ph.getLong(propFreeMemoryLimit, defaultFreeMemoryLimitMB, ParseExceptionAction.DefaultOnParseException) * ONEMB; 
        freeRAMLimit        = ph.getLong(propFreeRAMLimit, defaultFreeRAMLimitMB, ParseExceptionAction.DefaultOnParseException) * ONEMB; 
        freeSwapLimit          = ph.getLong(propFreeSwapLimit, defaultFreeSwapLimitMB, ParseExceptionAction.DefaultOnParseException) * ONEMB; 
        fastCheckThreshold     = ph.getLong(propFastCheckThreshold, defaultFastCheckThresholdMB, ParseExceptionAction.DefaultOnParseException) * ONEMB; 
        swapFastCheckThreshold = ph.getLong(propSwapFastCheckThreshold, defaultFastCheckThresholdMB, ParseExceptionAction.DefaultOnParseException) * ONEMB; 
        fastIntervalMillis     = ph.getInt(propFastIntervalMillis, defaultFastIntervalMillis, ParseExceptionAction.DefaultOnParseException); 
        slowIntervalMillis     = ph.getInt(propSlowIntervalMillis, defaultSlowIntervalMillis, ParseExceptionAction.DefaultOnParseException); 
    ...
    private void terminateLargest(boolean rssExceeded, boolean swapExceeded) throws IOException {
        ...
        if (processList.size() > 0) {
            ...
            String msg = new String(errorMessage +largest.getOwner() 
                    +"\t"+ largest.getStat().pid 
                    +"\t"+ largest.getStat().comm
                    +"\t" + largest.getStat().getRSSBytes());
            Log.warning(msg);
    ...
    private List<ProcessStatAndOwner> getCandidateProcessList() throws IOException {
        ...
        case USER_PRIORITY_AND_PROCESS_RSS:            
            prioritizedUserPatterns = ImmutableSet.copyOf(options.prioritizedUserPatterns.split(
                                                          ArgusOptions.prioritizedUserDelimiter));
        ...
        case USER_PRIORITY_AND_USER_RSS:
            ...
            break;
        default: throw new RuntimeException("Unexpected rssCandidateComparisonMode");
        }
