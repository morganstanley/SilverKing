...
public class SSUtil {
	...
	public static ByteBuffer retrievalResultBufferFromValue(ByteBuffer value, SSStorageParameters storageParams, SSRetrievalOptions options) {
		ByteBuffer	buf;
		boolean		returnValue;
		
		returnValue = options.getRetrievalType().hasValue();
		buf = ByteBuffer.allocate(MetaDataUtil.computeStoredLength(returnValue ? storageParams.getCompressedSize() : 0, storageParams.getChecksumType().length(), 
								emptyUserData.length));
		StorageFormat.writeToBuf(null, value, StorageParameters.fromSSStorageParameters(storageParams), emptyUserData, buf, new AtomicInteger(0), 
