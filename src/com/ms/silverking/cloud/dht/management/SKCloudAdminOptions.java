...
class SKCloudAdminOptions {
	...
	@Option(name="-c", aliases={"--command"}, usage="command. eg: \"LaunchInstances\", \"StartInstances\", \"StopInstances\", \"TerminateInstances\", \"StartSpark\", \"StopSpark\"", required=true)
	String command;
	...
	@Option(name="-e", aliases={"--exclude-master"}, usage="excludeMaster from being one of the \"numberOfInstances\"", required=false)
	boolean excludeMaster;
	
	@Option(name="-r", aliases={"--replication"}, usage="replication factor for the SilverKing cluster", required=false)
	int replication = 1;
