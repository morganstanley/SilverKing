...
public class GroupingSilverkingDB extends DB {
  ...
  static {
    ...
    for (int i = 0; i < numWorkers; i++) {
      //ThreadUtil.newDaemonThread(new Worker(session.openSyncNamespacePerspective(SilverkingDBConstants
      // .defaultNamespace, SilverkingDBConstants.nspOptions)), "Worker"+ i).start();
    }
  ...
  public void init() throws DBException {
    try {
      ...
      //syncNSP = session.openSyncNamespacePerspective(SilverkingDBConstants.defaultNamespace, SilverkingDBConstants
      // .nspOptions);
      //asyncNSP = session.openAsyncNamespacePerspective(SilverkingDBConstants.defaultNamespace,
      // SilverkingDBConstants.nspOptions);
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
  ...
  @Override
  public int scan(String table, String startKey, int recourdCount, Set<String> fields,
