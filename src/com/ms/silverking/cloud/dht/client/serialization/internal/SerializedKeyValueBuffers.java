...
public class SerializedKeyValueBuffers {
    ...
    private static final int    BYTES_PER_KEY = 2 * NumConversion.BYTES_PER_LONG 
    ...
    private int bufferOffsetOffset(int index) {
        return NumConversion.BYTES_PER_INT + 2 * NumConversion.BYTES_PER_LONG 
             + NumConversion.BYTES_PER_SHORT + index * BYTES_PER_KEY;
    }
    
    private int valueLengthOffset(int index) {
        return NumConversion.BYTES_PER_INT + 2 * NumConversion.BYTES_PER_LONG 
             + NumConversion.BYTES_PER_SHORT + NumConversion.BYTES_PER_INT 
             + index * BYTES_PER_KEY;
    }
    ...
    public DHTKey getKey(int index) {
        if (index >= numKeys) {
            ...
            return new SimpleKey(keyBuffer.getLong(mslOffset(index)), 
                                 keyBuffer.getLong(lslOffset(index)));
        }
    }
