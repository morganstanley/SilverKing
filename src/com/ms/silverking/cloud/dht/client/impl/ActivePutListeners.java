...
class ActivePutListeners {
  ...
  ActivePutListeners() {
    if (enableMultipleOpsPerMessage) {
      if (Libraries.useCustomGuava) {
        ...
        activePutListeners = Collections.synchronizedMap(
            new WeakHashMap<UUIDBase, ConcurrentMap<DHTKey,
                WeakReference<ActiveKeyedOperationResultListener<OpResult>>>>());
      }
  ...
  OperationUUID newOpUUIDAndMap() {
    ...
    if (enableMultipleOpsPerMessage) {
      activePutListeners.put(opUUID,
          new ConcurrentHashMap<DHTKey, WeakReference<ActiveKeyedOperationResultListener<OpResult>>>());
    }
    return opUUID;
  }
  ...
  public ConcurrentMap<DHTKey, WeakReference<ActiveKeyedOperationResultListener<OpResult>>> getKeyMap(
      OperationUUID opUUID) {
    if (enableMultipleOpsPerMessage) {
