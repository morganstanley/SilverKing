...
class ActiveRetrievalListeners {
  ...
  private final Map<UUIDBase, ConcurrentMap<DHTKey,
      List<WeakReference<ActiveKeyedOperationResultListener<MessageGroupRetrievalResponseEntry>>>>> activeRetrievalListeners;
  ...
  ActiveRetrievalListeners() {
    if (enableMultipleOpsPerMessage) {
      if (Libraries.useCustomGuava) {
        ...
        this.activeRetrievalListeners = Collections.synchronizedMap(
            new WeakHashMap<UUIDBase, ConcurrentMap<DHTKey,
  ...
  OperationUUID newOpUUID() {
    ...
    if (enableMultipleOpsPerMessage) {
      activeRetrievalListeners.put(opUUID,
          new ConcurrentHashMap<DHTKey,
              List<WeakReference<ActiveKeyedOperationResultListener<MessageGroupRetrievalResponseEntry>>>>(
              mapCapacity, keyMapConcurrencyLevel));
    }
    return opUUID;
  }
  ...
  Set<AsyncRetrievalOperationImpl> currentRetrievalSet() {
    ...
    if (enableMultipleOpsPerMessage) {
      for (ConcurrentMap<DHTKey,
          List<WeakReference<ActiveKeyedOperationResultListener<MessageGroupRetrievalResponseEntry>>>> map :
          activeRetrievalListeners.values()) {
        Log.fine("maps: ", map);
        // FUTURE - we can encounter concurrent modifications here
        // think about tolerating vs. preventing
        try {
          for (List<WeakReference<ActiveKeyedOperationResultListener<MessageGroupRetrievalResponseEntry>>> list :
              map.values()) {
