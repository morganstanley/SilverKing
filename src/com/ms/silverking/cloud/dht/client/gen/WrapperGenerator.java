...
public class WrapperGenerator {
   ...
   * @throws IOException
   */
  private void generate(List<GenPackageClasses> packageClassesList, File templateFile, File outputDir,
    ...
    if (addDependencies) {
      packageClassesList = GenPackageClasses.add(packageClassesList, referenceFinder.getReferences(),
          new File(options.codebase));
    }
   ...
   * @throws IOException
   */
  private void generate(Context c, List<ParseElement> elements, File outputDir, int loopIndex, int depth)
      throws IOException {
    int i;
        ...
        if (!isValidLoopTarget(c.getLoopElement(), le.getTarget())) {
          ...
          case Methods:
            ...
            for (Method m : getMethodsForGeneration(c.getClass_())) {
              ...
              if (!Modifier.isStatic(m.getModifiers()) && Modifier.isPublic(m.getModifiers())) {
                System.out.printf("Method\t%s\t%s\t%s\n", m.getName(), !JNIUtil.omitted(m),
                    referenceFinder.allReferencesPresent(m));
              }
              if (!Modifier.isStatic(m.getModifiers()) && Modifier.isPublic(m.getModifiers()) && !JNIUtil.omitted(
                  m) && referenceFinder.allReferencesPresent(m)) {
          ...
          case Constructors:
            for (Constructor constructor : c.getClass_().getConstructors()) {
              if (Modifier.isPublic(constructor.getModifiers()) && !JNIUtil.omitted(
                  constructor) && referenceFinder.allReferencesPresent(constructor)) {
   ...
   */
  private Quadruple<IfElement, List<ParseElement>, List<ParseElement>, Integer> getIfElements(
      List<ParseElement> elements, int if0Index) {
    ...
    for (int i = if0Index + 1; i < elements.size(); i++) {
      ...
      if (e instanceof IfElement) {
        ...
        switch (ie.getType()) {
        case If:
          throw new RuntimeException("Nested Ifs not supported");
        case Else:
          if (!inElse) {
            inElse = true;
          } else {
          ...
          break;
        case EndIf:
          return new Quadruple<>(ifElement, thenElements, elseElements, i + 1);
        default:
          throw new RuntimeException("Panic");
        }
      } else {
   ...
   * @return the ParseElements that are inside of the given if statement
   */
  private Triple<SwitchElement, List<Pair<CaseElement, List<ParseElement>>>, Integer> getSwitchElements(
      List<ParseElement> elements, int switchIndex) {
  ...
  public static void main(String[] args) {
    try {
      ...
      Variable.setTypeMappings(
          options.typeMappingFile == null ? ImmutableList.of() : TypeMapping.readTypeMappings(options.typeMappingFile));
      wg = new WrapperGenerator(options);
      wg.generate(
          GenPackageClasses.createForPackagesAndClasses(new File(options.codebase), options.inputPackagesAndClasses),
          new File(options.templateFile), new File(options.outputDir), options.addDependencies);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
