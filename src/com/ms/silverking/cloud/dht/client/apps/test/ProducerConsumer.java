...
public class ProducerConsumer {
  ...
  public ProducerConsumer(SKGridConfiguration gc, String id, Mode mode, int delay) throws ClientException, IOException {
    ...
    ns = session.createNamespace(id, session.getDefaultNamespaceOptions().storageType(StorageType.RAM).versionMode(
        NamespaceVersionMode.SEQUENTIAL).defaultWaitOptions(waitOptions));
  ...
  void consume(int i) throws RetrievalException {
    ...
    sv = syncNSP.retrieve(myKey,
        syncNSP.getOptions().getDefaultWaitOptions().versionConstraint(VersionConstraint.exactMatch(i)));
    if (i % displayUnit == 0) {
