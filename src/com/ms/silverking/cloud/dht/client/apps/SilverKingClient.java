...
public class SilverKingClient {
  ...
  public SilverKingClient(SessionOptions sessionOptions, InputStream in, PrintStream out, PrintStream err)
  ...
  private void doRetrieve(String[] args, RetrievalType retrievalType, WaitMode waitMode)
  ...
  private void displayMetaDataMap(Set<String> keys, Map<String, ? extends StoredValue<byte[]>> storedValues) {
    for (String key : sortedList(keys)) {
      ...
      out.printf("\n%s\n%s\n", key,
          storedValue != null ? MetaDataTextUtil.toMetaDataString(storedValue.getMetaData(), true) : noSuchValue);
    }
  }
  ...
  private Map<String, ? extends StoredValue<byte[]>> retrieveAllValuesForKey(String key) throws RetrievalException {
    ...
    retrievalOptions = syncNSP.getOptions().getDefaultGetOptions().retrievalType(
        RetrievalType.META_DATA).nonExistenceResponse(NonExistenceResponse.NULL_VALUE).versionConstraint(
        VersionConstraint.greatest).returnInvalidations(true);
    try {
      do {
        StoredValue<byte[]> storedValue;
        ...
        if (storedValue != null) {
          ...
          keyAndVersion = String.format("%s %d %d %s", key, storedValue.getVersion(),
              storedValue.getCreationTime().inNanos(), storedValue.getCreationTime().toDateString());
          keyValues.put(keyAndVersion, storedValue);
          //System.out.printf("%d\t%d\n", storedValue.getMetaData().getVersion(), storedValue.getMetaData()
          // .getCreationTime().inNanos());
          //retrievalOptions = retrievalOptions.versionConstraint(retrievalOptions.getVersionConstraint()
          // .maxCreationTime(storedValue.getCreationTime().inNanos() - 1));
          retrievalOptions = retrievalOptions.versionConstraint(
              retrievalOptions.getVersionConstraint().maxBelowOrEqual(storedValue.getVersion() - 1));
  ...
  private void doCommand(Action action, String[] args) throws IOException, OperationException {
    ...
    case PutRandom:
      ...
      break;
    case WaitFor:
      doRetrieve(args, valueFormat.getRetrievalType(), WaitMode.WAIT_FOR);
      break;
    case Get:
      doRetrieve(args, valueFormat.getRetrievalType(), WaitMode.GET);
      break;
    case Retrieve:
      doRetrieve(args, valueFormat.getRetrievalType());
      break;
    case Copy:
      doCopy(args);
      break;
    case GetMeta:
      doRetrieve(args, RetrievalType.META_DATA, WaitMode.GET);
      break;
    case GetAllValuesForKey:
      doRetrieveAllValuesForKey(args);
      break;
    ...
    case LinkToNamespace:
      doLinkToNamespace(args);
      break;
    case GetNamespaceOptions:
      doGetNamespaceOptions(args);
      break;
    case SetNamespace:
      doSetNamespace(args);
      break;
    ...
    case ToggleVerbose:
      ...
      break;
    case ValueMapDisplay:
      setValueMapDisplay(args);
      actionPerformed = false;
      break;
    case ValueFormat:
      setValueFormat(args);
  ...
  public static void main(String[] args) {
    try {
      ...
      if (options.gridConfig == null && options.clientDHTConfiguration == null && !SessionOptions.isReservedServerName(
          options.server)) {
      ...
      if (options.sessionTimeoutSeconds != 0) {
        ...
        timeoutController = new SimpleSessionEstablishmentTimeoutController(defaultSessionAttempts,
            maxTimeoutMillis / defaultSessionAttempts, maxTimeoutMillis);
  ...
  private class ChecksumAndLengthValueFormat implements ValueFormat {
    ...
    @Override
    public String valueToString(StoredValue<byte[]> value) {
      return value.getChecksumType() + "=" + StringUtil.byteArrayToHexString(
          value.getChecksum()) + ":length=" + value.getValue().length;
    }
  }
}
