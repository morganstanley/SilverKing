...
public class BulkThroughputAnalysis {
  ...
  public void runParallelTests_B(BulkThroughputTest test, TestParameters p, int externalReps)
  ...
  class TestRunner implements Runnable {
    ...
    TestRunner(BulkThroughputTest test, TestParameters p, int externalReps) throws ClientException {
      ...
      nsOptions = session.getDefaultNamespaceOptions().storageType(StorageType.RAM).consistencyProtocol(
          ConsistencyProtocol.LOOSE).defaultPutOptions(
    ...
    public int readAsync(TestParameters p, List<Double> batchTimes) throws RetrievalException {
      ...
      while (k <= p.maxKey) {
        ...
        while (!done) {
          ...
          if (curElapsed > 0.020) {
            out.println(
                new Date() + " " + System.currentTimeMillis() + " " + CollectionUtil.toString(latestResults.keySet()));
        ...
        if (batchSW.getElapsedSeconds() - lastKeyElapsed > 0.01) {
          System.out.printf("\t\tCompletion lag: %f\t%f\t%f\n", batchSW.getElapsedSeconds(), lastKeyElapsed,
              batchSW.getElapsedSeconds() - lastKeyElapsed);
        }
      }
      return readSize;
    ...
    public void runTest3(BulkThroughputTest test, TestParameters p, int externalReps)
        throws PutException, RetrievalException {
      List<Double> throughputList;
      List<Double> allBatchTimes;
      ...
      for (int j = 0; j < externalReps; j++) {
        ...
        for (int i = 0; i < p.repetitions; i++) {
          ...
          case Write:
            ...
            break;
          case Read:
            valueSize = read(p, batchTimes);
            break;
          case ReadAsync:
            valueSize = readAsync(p, batchTimes);
            break;
          default:
    ...
    public void runTest(BulkThroughputTest test, TestParameters p, int externalReps)
        throws PutException, RetrievalException {
      List<Double> throughputList;
      List<Double> allBatchTimes;
  ...
  private void storeReplicaTimes(String key, double curElapsed) throws RetrievalException {
  ...
  public static void main(String[] args) {
    try {
      ...
      bt = new BulkThroughputAnalysis(SKGridConfiguration.parseFile(options.gridConfig), System.out, System.err,
          options);
