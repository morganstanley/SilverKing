...
public class ProducerConsumer {
  ...
  ProducerConsumer(SKGridConfiguration gridConfig, String nsSuffix, int displayUnit)
      throws ClientException, IOException {
    syncNSP = new DHTClient().openSession(gridConfig).openSyncNamespacePerspective(pcNamespace + nsSuffix,
  ...
  public void run(Mode mode, int startKey, int endKey) throws ClientException {
    switch (mode) {
    case Producer:
      producer(startKey, endKey);
      break;
    case Consumer:
      consumer(startKey, endKey);
      break;
    default:
