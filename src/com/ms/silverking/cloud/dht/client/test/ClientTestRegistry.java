        ...
        for (NamespaceVersionMode vm : NamespaceVersionMode.values()) {
            for (RevisionMode rm : RevisionMode.values()) {
                for (ConsistencyProtocol cp : ConsistencyProtocol.values()) {
                    /**/
                    addParameterizedNamespaceTest(tests, DHTConstants.defaultNamespaceOptions.versionMode(vm).revisionMode(rm).consistencyProtocol(cp).storageFormat("0"));
                    addParameterizedNamespaceTest(tests, DHTConstants.defaultNamespaceOptions.versionMode(vm).revisionMode(rm).consistencyProtocol(cp).storageFormat("2"));
                    /**/
                    /*
                    if (cp == ConsistencyProtocol.LOOSE) {
                        ...
                        if (vm == NamespaceVersionMode.SINGLE_VERSION && rm == RevisionMode.NO_REVISIONS) {
                            // single version/no revision not supported for backward compatibility (old server recovering data from new server)
                        } else {
                            addParameterizedNamespaceTest(tests, DHTConstants.defaultNamespaceOptions.versionMode(vm).revisionMode(rm).consistencyProtocol(cp));
                        }
                    }
                    */
                }
