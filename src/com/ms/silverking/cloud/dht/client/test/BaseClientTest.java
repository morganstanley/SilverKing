...
public abstract class BaseClientTest implements ClientTest {
    ...
    public BaseClientTest(String testName, Set<Triple<ConsistencyProtocol,NamespaceVersionMode,RevisionMode>>  nsOptionsParameters) {
    ...
    public List<NamespaceOptions> getNamespaceOptions(NamespaceOptions sessionDefaultNSOptions) {
        ...
        for (Triple<ConsistencyProtocol,NamespaceVersionMode,RevisionMode> nsop : nsOptionsParameters) {
            nsOptions.add(sessionDefaultNSOptions.consistencyProtocol(nsop.getV1()).versionMode(nsop.getV2()).revisionMode(nsop.getV3()));
        }
    ...
    protected void checkValueAndVersion(SynchronousNamespacePerspective<String,String> syncNSP, String key, String expectedValue, long expectedVersion) throws RetrievalException {
        StoredValue<String> sv;
        String      value;
        
        System.out.printf("Reading %s\n", key);
        sv = syncNSP.retrieve(key, syncNSP.getOptions().getDefaultGetOptions().retrievalType(RetrievalType.VALUE_AND_META_DATA));
        value = sv.getValue();
        System.out.printf("value: %s\tversion %d\n", value, sv.getMetaData().getVersion());
        if (sv.getMetaData().getVersion() != expectedVersion) {
            throw new RuntimeException(String.format("Expected version: %d\tFound: %d", expectedVersion, sv.getMetaData().getVersion()));
        }
        if (!ObjectUtil.equal(value, expectedValue)) {
            throw new RuntimeException(String.format("Expected: %s\tFound: %s", expectedValue, value));
        }
    ...
    protected void checkInvalidatedVersion(SynchronousNamespacePerspective<String,String> syncNSP, String key, long expectedVersion) throws RetrievalException {
        StoredValue<String> sv;
        
        System.out.printf("Reading %s\n", key);
        sv = syncNSP.retrieve(key, syncNSP.getOptions().getDefaultGetOptions().retrievalType(RetrievalType.META_DATA).returnInvalidations(true));
        System.out.printf("isInvalidation %s version %d\n", sv.getMetaData().isInvalidation(), sv.getMetaData().getVersion());
