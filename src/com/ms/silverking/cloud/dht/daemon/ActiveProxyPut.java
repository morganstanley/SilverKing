...
class ActiveProxyPut extends ActiveProxyOperation<MessageGroupKeyEntry, PutResult> implements PutOperationContainer {
  ...
  ActiveProxyPut(MessageGroup message, ByteBuffer optionsByteBuffer, MessageGroupConnectionProxy connection,
    ...
    if (_version == DHTConstants.unspecifiedVersion) {
      ...
      case SYSTEM_TIME_MILLIS:
        _version = systemTimeSource.absTimeMillis();
        break;
      case SEQUENTIAL: // FIXME - for now we just use nanos for sequential version
      case SYSTEM_TIME_NANOS:
        _version = systemTimeSource.absTimeNanos();
        break;
      default:
        throw new RuntimeException("non-system or unexpected VersionMode: " + nsOptions.getVersionMode());
      }
    ...
    storageOperation = storageProtocol.createStorageOperation(
        message.getDeadlineAbsMillis(messageModule.getAbsMillisTimeSource()), this, message.getForwardingMode());
  ...
  void startOperation() {
    ...
    forwardGroupedEntries(pComm.takeReplicaUpdateMessageLists(), optionsByteBuffer,
        new PutUpdateForwardCreator(storageOperation.nextStorageState(storageOperation.initialStorageState())), pComm);
  ...
  protected void localOp(List<? extends DHTKey> _entries, OpCommunicator<MessageGroupKeyEntry, PutResult> comm) {
    ...
    if (!useUpdate) {
      ...
      for (DHTKey _entry : _entries) {
        values.add(
            new StorageValueAndParameters((MessageGroupPutEntry) _entry, (PutOperationContainer) this, creationTime,
      ...
      if (forwardingMode.forwards()) {
        for (DHTKey _entry : _entries) {
          storageOperation.localUpdate(_entry, StorageProtocolUtil.initialStorageStateOrdinal, OpResult.SUCCEEDED,
              pComm);
        }
      }
    } else {
  ...
  OpResult handlePutResponse(MessageGroup message, MessageGroupConnectionProxy _connection) {
    ...
    if (rumLists.size() > 0) {
      forwardGroupedEntries(rumLists, optionsByteBuffer,
          new PutUpdateForwardCreator(storageOperation.nextStorageState(storageState)), pComm);
    }
