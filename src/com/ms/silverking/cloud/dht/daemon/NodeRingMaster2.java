...
public class NodeRingMaster2 implements DHTMetaUpdateListener, KeyToReplicaResolver {
  ...
  public void dhtMetaUpdate(DHTMetaUpdate dhtMetaUpdate) {
    ...
    public void newCurRingAndVersion(Triple<String,Long,Long> curRingAndVersionPair) {
        ...
        try {
            if (curMapState == null) {
                ...
                if (mapState == null) {
                    Log.warning("Unexpected can't find RingMapState for ringIDAndVersionPair in in mapStates: "+
                    ringIDAndVersionPair);
                    return;
                }
                curMapState.discard();
            }
            ...
            if (!convergenceEnabled) {
                ...
                if (targetRingIDAndVersionPair.equals(curMapState.getRingIDAndVersionPair())) {
                    ...
                    if (dhtMetaUpdate.getRingIDAndVersionPair().equals(curMapState.getRingIDAndVersionPair())) {
                        Log.warning("Ignoring (2) newTargetRingAndVersion with ringIDAndVersionPair same as
                        curMapState");
                        return;
                    }
    ...
    public RingMapState2 createMapState(DHTMetaUpdate dhtMetaUpdate, boolean convergenceAlreadyComplete) {
        try {
            ...
            storagePolicyGroupVersion = zk.getVersionPriorTo(ringMC.getMetaPaths().getStoragePolicyGroupPath(),
            zkidLimit);
            ...
            return new RingMapState2(nodeID, dhtMetaUpdate, ringID, 
                                           storagePolicyGroup, ringMC, ExclusionSet.emptyExclusionSet(0)
                                           latestExclusionSet, this, dhtMC,
  ...
  private RingMapState2 getMapState(RingIDAndVersionPair ringIDAndVersion) {
    ...
    } else if (targetMapState != null && targetMapState.getConvergencePoint().getRingIDAndVersionPair().equals(
        ringIDAndVersion)) {
  ...
  public void initializeMap(DHTConfiguration dhtConfig) throws IOException, KeeperException {
    ...
    curRingAndVersionPair = new DHTRingCurTargetZK(dhtMetaReader.getMetaClient(),
        dhtMetaReader.getDHTConfig()).getCurRingAndVersionPair();
    ...
    currentCP = new ConvergencePoint(dhtConfig.getVersion(),
        RingIDAndVersionPair.fromRingNameAndVersionPair(curRingAndVersionPair), creationTime);
  ...
  public void setConvergenceState(MessageGroup message, MessageGroupConnection connection) {
    ...
    //} else {
    if (targetMapState == null || (!targetMapState.getConvergencePoint().equals(
        targetCP) && state != RingState.ABANDONED)) {
