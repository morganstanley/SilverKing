...
public class RingMapState implements NCGListener, PeerStateListener {
  ...
  RingMapState(IPAndPort nodeID, DHTMetaUpdate dhtMetaUpdate, RingID ringID, StoragePolicyGroup storagePolicyGroup,
    ...
    if (!convergenceAlreadyComplete) {
      try {
        peerStateWatcher = new PeerStateWatcher(getResolvedReplicaMap().allReplicas(), dhtMetaUpdate.getMetaClient(),
            dhtMetaUpdate.getDHTConfig(), ringIDAndVersionPair.getRingVersionPair(), this);
  ...
  private void readInitialExclusions(MetaClient mc) throws KeeperException {
    ...
    try {
      newRingTree = RingTreeBuilder.removeExcludedNodes(rawRingTree,
          ExclusionSet.union(curExclusionSet, curInstanceExclusionSet));
    } catch (Exception e) {
      Log.logErrorWarning(e);
      throw new RuntimeException(e);
    }
