...
public class TimeBasedSegmentRollOver {
  ...
  public static final String timeThresholdMillisProp = "com.ms.silverking.cloud.dht.daemon.storage" +
      ".timeBasedSegmentRollOverThresholdMillis";
  ...
  public static boolean needRollOver(WritableSegmentBase currSegment) {
    if (isEnabled) {
    if (timeThresholdMillis < 0) {
        Log.warning(
            "TimeBasedSegmentRollOver's timeThresholdMillis is negative - it may hasn't been set via property:" + timeThresholdMillisProp);
      return false;
    } else {
