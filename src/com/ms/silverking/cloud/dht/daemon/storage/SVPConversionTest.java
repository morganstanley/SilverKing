...
public class SVPConversionTest {
    ...
    private StorageValueAndParameters convertToMappedSVP(StorageValueAndParameters svp) throws IOException {
        ...
        msvp = new StorageValueAndParameters(svp.getKey(), 
                convertToMappedBuffer(svp.getValue()),
                svp.getVersion(), svp.getUncompressedSize(), svp.getCompressedSize(),
                svp.getCCSS(), svp.getChecksum(), 
                svp.getValueCreator(), svp.getCreationTime(),
                svp.getRequiredPreviousVersion(), svp.getLockSeconds());
        return msvp;
    }
    ...
    public StorageValueAndParameters createSVP(int valueSize) {
        return new StorageValueAndParameters(new SimpleKey(1, 2), 
                ByteBuffer.allocate(valueSize),
                10, valueSize, valueSize,
                (short)0, new byte[4], 
                new byte[ValueCreator.BYTES], System.currentTimeMillis(),
                0, (short)0);
    }
