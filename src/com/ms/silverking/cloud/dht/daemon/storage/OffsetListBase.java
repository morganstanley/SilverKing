...
abstract class OffsetListBase implements OffsetList {
  ...
  private int getOffset_linear(VersionConstraint vc, ValidityVerifier validityVerifier) {
    ...
    for (int i = 0; i < size(); i++) {
      ...
      if (supportsStorageTime && vc.getMaxCreationTime() != VersionConstraint.noCreationTimeLimit && vc.getMaxCreationTime() < getStorageTime(
          i)) {
        if (debug) {
  ...
  public static int entrySizeBytes(boolean supportsStorageTime) {
    return (supportsStorageTime ?
        entrySizeInts_supportsStorageTime :
        entrySizeInts_noStorageTime) * NumConversion.BYTES_PER_INT;
  }
}
