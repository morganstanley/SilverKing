...
public class FileSegmentMetaData {
    ...
    public LTVElement getElement(FSMElementType type) {
        ...
        if (fsStorageFormat.dataSegmentIsLTV()) {
            ...
            switch (type) {
            case OffsetMap: return new KeyToOffsetMapElement(elementBuf);
            case OffsetLists: return new OffsetListsElement(elementBuf);
            case InvalidatedOffsets: return new InvalidatedOffsetsElement(elementBuf);
            default: return null;
            }
        } else {
            switch (type) {
    ...
    public static FileSegmentMetaData read(FileSegmentStorageFormat fsStorageFormat, RandomAccessFile raFile, long dataSegmentSize, MetaDataPrereadMode prereadMode) throws IOException {
        ByteBuffer  fsmBuf;
    ...
    public static void persist(FileSegmentStorageFormat fsStorageFormat, RandomAccessFile raFile, int dataSegmentSize, 
                               IntArrayCuckoo keyToOffset, RAMOffsetListStore offsetListStore, Set<Integer> invalidatedOffsets) throws IOException {
        ...
        if (fsStorageFormat.containsFSMHeader()) {
            ...
            fsmLength = fsmHeaderElement.getLength() + keyToOffsetMapElement.getLength() + offsetListsElement.getLength() + invalidatedOffsetsElement.getLength();
        } else {
            fsmHeader = null;
            fsmHeaderElement = null;
            fsmLength = keyToOffsetMapElement.getValueLength() + KeyToOffsetMapElementV0.headerSize + offsetListsElement.getValueLength();
        }
