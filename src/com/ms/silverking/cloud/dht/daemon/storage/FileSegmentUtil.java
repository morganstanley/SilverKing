...
public class FileSegmentUtil {
  ...
  public void debugSegment(DHTKey key, File nsDir, int segmentNumber, NamespaceOptions nsOptions, boolean displayValues)
      throws IOException {
    FileSegment segment;
    ...
    for (DHTKeyIntEntry entry : segment.getPKC()) {
      //System.out.printf("%s\t%s\t%s\n", KeyUtil.keyToString(entry.getKey()), KeyUtil.keyToString(key), entry.getKey
      // ().equals(key));
  ...
  public static void debugFiles(DHTKey key, File nsDir, int minSegment, int maxSegment, boolean displayValues)
      throws IOException {
