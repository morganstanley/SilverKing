...
public class PeerStateWatcher implements ChildrenListener {
  ...
  public PeerStateWatcher(Set<IPAndPort> peers, com.ms.silverking.cloud.dht.meta.MetaClient dhtMC,
      DHTConfiguration dhtConfig, Pair<Long, Long> ringVersionPair, PeerStateListener peerStateListener,
      RingState targetState) throws KeeperException {
    active = true;
    this.peers = peers;
    ringStateZK = new RingStateZK(dhtMC, dhtConfig, ringVersionPair);
    childrenWatcher = new ChildrenWatcher(RingMapState.peerStateWatcherTimer, dhtMC,
        ringStateZK.getRingInstanceStatePath(), this, RingMapState.peerStateCheckIntervalMillis,
        RingMapState.peerStateInitialIntervalMillis);
      ...
      if (childStates.size() >= peers.size()) {
        ...
        for (IPAndPort node : nodesToCheck) {
          ...
          if (!targetState.metBy(nodeState)) {
            //if (debug) {
            Log.warning(String.format("targetState not found for: %s nodeState %s targetState %s %s", node.toString(),
                nodeState, targetState, targetState.metBy(nodeState)));
            if (nodeState == null) {
