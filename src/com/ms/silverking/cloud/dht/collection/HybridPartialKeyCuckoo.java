...
public class HybridPartialKeyCuckoo extends PartialKeyIntCuckooBase {
  ...
  protected HybridPartialKeyCuckoo(WritableCuckooConfig cuckooConfig, ExternalStore externalStore, long[] ht,
      boolean initialize) {
    super(cuckooConfig, externalStore, new SubTable[cuckooConfig.getNumSubTables()]);
    this.ht = ht;
    for (int i = 0; i < subTables.length; i++) {
      subTables[i] = new SubTable(ht, subTableBuckets * entriesPerBucket * i, subTableBuckets,
          cuckooConfig.getEntriesPerBucket(), i);
    }
    if (initialize) {
      initialize();
    }
  ...
  public HybridPartialKeyCuckoo(WritableCuckooConfig cuckooConfig, ExternalStore externalStore) {
  ...
  private static HybridPartialKeyCuckoo rehash(PartialKeyIntCuckooBase oldTable) {
    ...
    newTable = new HybridPartialKeyCuckoo(oldTable.getConfig().doubleEntries(), oldTable.getExternalStore());
    try {
