...
public class PartialKeyCuckoo extends PartialKeyIntCuckooBase {
  protected PartialKeyCuckoo(WritableCuckooConfig cuckooConfig, ExternalStore externalStore, ByteBuffer hashTableBuffer,
      boolean initialize) {
    super(cuckooConfig, externalStore, new SubTable[cuckooConfig.getNumSubTables()]);
    for (int i = 0; i < subTables.length; i++) {
      LongBuffer htBuf;

      htBuf = ((ByteBuffer) hashTableBuffer.duplicate().position(
          subTableBuckets * entriesPerBucket * i * NumConversion.BYTES_PER_LONG)).order(
  ...
  public PartialKeyCuckoo(WritableCuckooConfig cuckooConfig, ExternalStore externalStore, ByteBuffer hashTableBuffer) {
