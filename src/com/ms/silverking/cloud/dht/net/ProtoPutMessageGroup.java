...
public final class ProtoPutMessageGroup<V> extends ProtoValueMessageGroupBase {
  ...
  public ValueAdditionResult addValue(DHTKey dhtKey, V value) {
    ...
    if (compressor != null || encrypterDecrypter != null) {
      ...
      if (serializedBytes.limit() != 0) {
        try {
          if (compressor != null) {
            bytesToStore = compressor.compress(serializedBytes.array(), serializedBytes.position(),
    ...
    addValueHelper(dhtKey, _bufferIndex, _bufferPosition, uncompressedValueSize, compressedValueSize,
        bytesToChecksumBuf);
  ...
  private void addValueHelper(DHTKey dhtKey, int _bufferIndex, int _bufferPosition, int uncompressedValueSize,
      int compressedValueSize, ByteBuffer bytesToChecksumBuf) {
    // key byte buffer update
    addKey(dhtKey);
