...
public class DataMigrationTool {
    ...
    public void walk(File nsDir, File ssDir, int segmentNumber, Set<DHTKey> filteredPaths) throws IOException {
        ...
        while (dsWalker.hasNext()) {
            ...
            entry = dsWalker.next();
            System.out.println(entry.getOffset() +" "+ entry  +"\t"+ new Date(SystemTimeUtil.systemTimeNanosToEpochMillis(entry.getCreationTime())));            
    ...
    private void migrateEntry(DataSegmentWalkEntry entry, File ssDir, Set<DHTKey> filteredPaths) throws IOException {
        ...
        if (!filteredPaths.contains(key)) {
            ...
            newSP = new StorageParameters(o.getVersion(), value.length, value.length, o.getLockSeconds(), CCSSUtil.createCCSS(Compression.NONE, o.getChecksumType(), o.getStorageState()), o.getChecksum(), o.getValueCreator(), o.getCreationTime());
