...
class ClassParser<T> {
    ...
    ClassParser(Class _class, T template,
        ...
        try {
            ...
            if (debug) {
                System.out.printf("exclusionFields %s\n", CollectionUtil.toString(this.exclusionFields));
            }
            fields = CPUtils.filterFields(CPUtils.filterStaticFields(CPUtils.getDeclaredFields(_class)), this.exclusionFields);
            if (debug) {
                System.out.printf("fields %s\n", ArrayUtil.toString(fields));
            }
    ...
    ClassParser(T template,
            ...
            Set<String> optionalFields,
            Set<String> exclusionFields) {
        this(template.getClass(), template, fieldsRequirement, nonFatalExceptionResponse,
    ...
    ClassParser(T template,
            ...
            Set<String> optionalFields,
            Set<String> exclusionFields) {
        this(template, fieldsRequirement, nonFatalExceptionResponse, 
             Character.toString(defaultFieldDefDelimiter), Character.toString(defaultNameValueDelimiter), 
             optionalFields, exclusionFields);
    }
    ...
    private Object[] createConstructorArgsFromParams(Map<String,String> defMap) {
        ...
        if (debug) {
            System.out.printf("\ncreateConstructorArgs %s\n", CollectionUtil.mapToString(defMap));
        }
                ...
                try {
                    ...
                    if (nonFatalExceptionResponse != NonFatalExceptionResponse.THROW_EXCEPTIONS) {
                        try {
                            ...
                            throw new ObjectDefParseException("Unable to set field value from template in "
                                                               + _class.getName(), e2);
                        }
                    } else {
                        throw e;
    ...
    public T parse(String fieldDefs) {
        ...
        if (debug) {
            System.out.println("\n################################################################");
            System.out.println("\nparse "+ _class.getName() +"["+ fieldDefs +"]");
        }
        ...
        for (String fieldDef : splitFieldDefs(fieldDefs)) {
            ...
            if (fieldDef.length() > 0) {
                ...
                Log.warning("Ignoring empty field def in "+ _class);
            }
        }
        try {
    ...
    public String objectToString(T obj, Set<String> dedicatedExclusions) {
        Class   superClass;
        ...
        if (dedicatedExclusions == null) {
            dedicatedExclusions = exclusionFields;
        }
        fields = CPUtils.filterFields(CPUtils.filterStaticFields(_class.getDeclaredFields()), dedicatedExclusions);
        for (int i = 0; i < fields.length; i++) {
            Field   field;
            boolean recursive;
            String  valueString;
