...
public class AsyncPingMultiPong {
  ...
  public AsyncPingMultiPong(PingPongOptions options) throws IOException, ClientException {
    ...
    nsOptions = session.getDefaultNamespaceOptions().storageType(StorageType.RAM).consistencyProtocol(
        ConsistencyProtocol.TWO_PHASE_COMMIT).versionMode(NamespaceVersionMode.CLIENT_SPECIFIED);
    ...
    if (options.checksumType != null) {
      defaultPutOptions = session.getDefaultNamespaceOptions().getDefaultPutOptions().checksumType(
          ChecksumType.valueOf(options.checksumType));
    } else {
  ...
  public void clientIteration(String pingKey, String pongKeyBase, long version) throws OperationException {
    ...
    asyncRetrieval = asyncNSP.retrieve(pongKeys,
        asyncNSP.getOptions().getDefaultWaitOptions().versionConstraint(VersionConstraint.exactMatch(version)));
  ...
  public void serverIteration(String pingKey, String pongKeyBase, long version, int threadID)
    ...
    asyncWaitFor = asyncNSP.retrieve(pingKey,
        asyncNSP.getOptions().getDefaultWaitOptions().versionConstraint(VersionConstraint.exactMatch(version)));
