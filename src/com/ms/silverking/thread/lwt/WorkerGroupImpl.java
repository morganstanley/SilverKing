...
class WorkerGroupImpl<I> implements WorkerGroup<I> {
	...
	public WorkerGroupImpl(String name, 
				int maxDirectCallDepth, int idleThreadThreshold) {
		if (name != null) {
			this.name = name;
		} else {
	...
	public WorkerGroupImpl(String name, Collection<BaseWorker<I>> workers, int maxDirectCallDepth, int idleThreadThreshold) {
		this(name, maxDirectCallDepth, idleThreadThreshold);
		for (BaseWorker<I> worker : workers) {
			addWorker(worker);
		}
	...
	public void addWork(I item) {
		workers.get(Math.abs(nextWorker.getAndIncrement() % workers.size()))
