...
public class BlockingQueueTest implements Runnable {
	...
	public BlockingQueueTest(int readers, int writers,
	                        long readerDelayNanos, long writerDelayNanos,
	...
	private void displayResults() {
		...
		for (int i = 0; i < workComplete.length; i++) {
			...
			System.out.printf("%d\t%s\t%d\t%e\n", i, isReader[i], workComplete[i], (sw.getElapsedSeconds() / (double)workComplete[i]));
			//System.out.println(i +"\t"+ (isReader[i] ? "reader" : "writer") +"\t"+ workComplete[i]);
		}
        System.out.printf("\nReads:\t%d\t%e\n", readsComplete, (sw.getElapsedSeconds() / (double)readsComplete));
	}
	...
	public static void main(String[] args) {
		try {
			...
			if (args.length != 6) {
				System.out.println("args: <readers> <writers> <duration> <readerDelayNanos> <writerDelayNanos> "+
