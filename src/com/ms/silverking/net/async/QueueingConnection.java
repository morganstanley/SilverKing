      ...
      while (!outputQueue.isEmpty()) {
        ...
        if (outgoingData != null) {
          try {
            ...
            //if (verbose) {
            Log.warning(String.format("send failed due to exception: %s  remote: %s", ioe, getRemoteSocketAddress()));
            //}
  ...
  public void sendAsynchronous(Object data, UUIDBase sendID, AsyncSendListener asyncSendListener, long deadline)
    ...
    try {
      ...
      if (locked && outputQueue.isEmpty()) {
        ...
        if (debug) {
          Log.warning("sendAsynchronous result ",
